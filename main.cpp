	...
	while (true)
	{
		...
		if (pMsg != NULL)
		{
			...
			case d:
				...
				cout << "high:" << ptest->high << endl;
				cout << "tempture:" << ptest->tempture << endl;
				cout << "length:" << ptest->length << endl;
				int *a = (int*)ptest->p;
				for (int i = 0; i < 10; i++)
					cout << a[i] << endl;
				free(ptest->p);
...
int main()
{
	...
	tmain = new ThreadMsg(pthread_self());
	tmain->start();
	...
	while (true)
	{
		...
		//查询自己的队列中有消息没，如果有就处理
		pmsg = tmain->get_msg(false);
		main_process_message(pmsg);
		break;
	}
